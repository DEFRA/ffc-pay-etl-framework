...
const { UniqueValidator } = require('../../app/validators/uniqueValidator')
    ...
    readable
      ...
      .pipe(new PassThrough({
        ...
        transform (chunk, _, callback) {
          ...
          rowCount += 1
          if (rowCount === testData.length) { done() }
          callback(null, chunk)
        }
      })
      )
  })
  test('should add errors if specified field is not unique', (done) => {
    const uut = UniqueValidator({ column: 'field_one' })
